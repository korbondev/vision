...
async def get_image_from_server(body: BaseModel, post_endpoint: str, timeout: float = 20.0):
    ...
    async with aiohttp.ClientSession(raise_for_status=True) as session:
        try:
            async with session.post(endpoint, json=body.dict()) as response:
                data = await response.json()
                image_response = utility_models.ImageResponseBody.construct(**data)
                return image_response
        except Exception as exc:
            bt.logging.warning(
                f"Error when getting an image while making request to {endpoint}: {exc}"
            )
