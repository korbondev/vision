...
ORGANIC_ENDPOINT = os.getenv("VALI_ORGANIC_ENDPOINT")
headers = {
    "Authorization": f"Bearer {API_KEY}",
}
init_image = image_to_base64("tests/assets/tall_man.webp")

BASE_URL = ORGANIC_ENDPOINT
...
#         width=1280,
#     )
#     response = requests.post(BASE_URL + "image-to-image", json=data.dict(), headers=headers)
#     assert response.status_code == 200
...
#     )
#     response = requests.post(BASE_URL + "inpaint", json=data.dict(), headers=headers)
#     assert response.status_code == 200
...
#         width=1024,
#     )
#     response = requests.post(BASE_URL + "avatar", json=data.dict(), headers=headers)
